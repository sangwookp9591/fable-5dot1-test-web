...
| 섹션 | 길이 | 입력 | 반응 |
|---|---|---|---|
| intro | 100vh | [바로 보기] 클릭 | `started` → 모니터 켜짐, career 로 한 번 스크롤, 섹션 dot 표시 |
| career | 180vh | 스크롤 12/30/48/66% · 행 hover | 보드 행이 채워짐(DOM + 3D 벽 보드 동기) · hover 시 그 회사에서 한 일 펼침 |
| zivo | 260vh | 스크롤 5~40% · 슬라이더 드래그 | 숫자 카운트업 · 45~90% 에서 쿠키→URL 전환, 드래그로 앞당길 수 있음, 50% 넘으면 아잉 surprise |
| loop | 320vh | 스크롤 28/52/76% | 터미널 내용 · 단계 카드 · 모니터 텍스처 · 아잉 think→review→type→celebrate |
| studio | 300vh | 스크롤 · 정거장 버튼 | 카메라 4 정거장, TV 타일 점등, 서버 랙 분리, 카드 교체. 버튼은 해당 스크롤 위치로 이동 |
| ai | 160vh | 스크롤 15/50% | 카드 stagger, 순서 칩 |
| review | 160vh | 자동 | PerformanceObserver 로 LCP/CLS/INP, rAF 로 FPS 측정. 기준 미달이면 아잉 error + "이건 좀 느린데?" |
| result | 140vh | 스크롤 86% · 버튼 | 아잉 마지막 대사 후 leave 클립, [다시 보기] 는 맨 위로 |
